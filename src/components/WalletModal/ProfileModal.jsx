...
// ==========================
// ✅ VALIDADORES (CPF / DATA)
// ==========================
function digitsOnly(v) {
  return String(v || "").replace(/\D+/g, "");
}
function isValidCpfDigits(cpfDigits) {
  const s = digitsOnly(cpfDigits);
...
function isValidISODate(iso) {
  ...
  // garante que o parse não “corrigiu” a data (ex: 2026-02-31)
...
function isValidBirthISO(iso) {
  ...
  // mínimo razoável (evita 0001-01-01 etc)
...
export default function ProfileModal({
  ...
  // ✅ overlay global (Dominion Black + spinner)
  const { pushGlobalLoading, popGlobalLoading } = useUILoading();
  ...
  // ✅ refs p/ forçar foco nos campos com erro
  ...
  // ✅ erros por campo (sem mexer em layout)
  const [fieldErrors, setFieldErrors] = useState({ cpf: "", nascimento: "" });
  ...
  const portalTarget = useMemo(() => {
    ...
    if (isTradeMode && host && !isMobileViewport) return host;
    return document.body;
  }, [isTradeMode, isMobileViewport]);
  ...
  const setField = (k, v) => {
    ...
    if (k === "cpf") {
      setFieldErrors((e) => (e.cpf ? { ...e, cpf: "" } : e));
    }
    if (k === "nascimento") {
      setFieldErrors((e) => (e.nascimento ? { ...e, nascimento: "" } : e));
    }
    setForm((s) => ({ ...s, [k]: v }));
  };
  ...
  const doSave = async () => {
    ...
    // ✅ validações locais (bloqueia salvar CPF/Data inválidos)
    setFieldErrors((e) => (e.cpf || e.nascimento ? { cpf: "", nascimento: "" } : e));

    const cpfRaw = (form.cpf || "").trim();
    if (!cpfLocked) {
      const cpfDigits = digitsOnly(cpfRaw);
      if (cpfDigits.length > 0 && !isValidCpfDigits(cpfDigits)) {
        setFieldErrors((e) => ({ ...e, cpf: t("profile:validation.cpf_invalid") }));
        setSaveState("idle");
        setSaveMsg("");
        requestAnimationFrame(() => cpfInputRef.current?.focus?.());
        return;
      }
    }

    const birthRaw = (form.nascimento || "").trim();
    if (!birthLocked && !profile?.birth_date && birthRaw) {
      if (!isValidBirthISO(birthRaw)) {
        setFieldErrors((e) => ({ ...e, nascimento: t("profile:validation.birth_invalid") }));
        setSaveState("idle");
        setSaveMsg("");
    ...
    const cpfToSend = cpfLocked ? undefined : cpfDigits.length ? cpfDigits : null;
    ...
    if (error) {
      ...
      const emsgLow = emsg.toLowerCase();
      const ecode = String(error?.code || "");

      // ✅ CPF já cadastrado (unique violation) -> erro no campo + foco
      if ((ecode === "23505" || emsgLow.includes("duplicate") || emsgLow.includes("unique")) && emsgLow.includes("cpf")) {
        console.warn("[ProfileModal] cpf unique violation:", emsg);
        setFieldErrors((e) => ({ ...e, cpf: t("profile:validation.cpf_in_use") }));
        setSaveState("idle");
        setSaveMsg("");
        requestAnimationFrame(() => cpfInputRef.current?.focus?.());
        return;
      ...
      console.warn("[ProfileModal] save error:", emsg);
      setSaveState("error");
  ...
  const canChangePasswordFilled = Boolean(oldPass && newPass && newPass2);
  const passwordsMatch = newPass === newPass2;
  // ✅ UX: quando o erro é "senhas não coincidem", mostra a mensagem apenas 1x (no campo de confirmação)
  const isPasswordMismatchError = Boolean(passErrors?.new && passErrors?.confirm && passErrors.new === passErrors.confirm);

  const doChangePassword = async () => {
    SoundManager.uiClick();
    if (!canChangePasswordFilled) return;
    if (passState === "saving") return;

    // ✅ validação: senhas precisam coincidir (marca campos + foco)
    setPassErrors((e) => (e.old || e.new || e.confirm ? { old: "", new: "", confirm: "" } : e));
    if (!passwordsMatch) {
      const msg = t("profile:security.passwords_not_match");
      setPassErrors({ old: "", new: msg, confirm: msg });
      setPassState("error");
      setPassMsg(msg);
      requestAnimationFrame(() => newPass2Ref.current?.focus?.());
      return;
    }
    ...
    try {
      ...
      if (reauthErr) {
        ...
        setPassState("error");
        const msg = t("profile:security.invalid_current_password");
        setPassMsg(msg);
        setPassErrors((e) => ({ ...e, old: msg }));
        requestAnimationFrame(() => oldPassRef.current?.focus?.());
        if (passTimerRef.current) clearTimeout(passTimerRef.current);
  ...
  // ✅ ref com o avatarUrl mais recente (evita stale closure dentro do async do upload)
  const avatarUrlRef = useRef("");
  useEffect(() => {
    avatarUrlRef.current = avatarUrl || "";
  }, [avatarUrl]);

  // ✅ evita "flash"/quadrado preto enquanto o upload está acontecendo
  const [avatarPreviewUrl, setAvatarPreviewUrl] = useState("");
  useEffect(() => {
    // Só atualiza automaticamente quando NÃO está enviando
    if (!avatarBusy) {
      setAvatarPreviewUrl(avatarBust || "");
    }
  }, [avatarBust, avatarBusy]);

  const preloadImage = useCallback((url, timeoutMs = 2500) => {
  ...
  const onPickAvatar = async (e) => {
    ...
    const uid = effectiveUser?.id;
    if (!uid) return;
    // ✅ fase 1: mostrar "Enviando..." por ~500–600ms
    setAvatarBusy(true);
    setAvatarMsg(t("profile:actions.sending"));

    const SEND_MIN_MS = 560;
    let overlayShown = false;
    let overlayTimer = null;
    let sendStart = Date.now();

    // Após o mínimo, entra o loading global (se ainda estiver enviando)
    overlayTimer = setTimeout(() => {
      try {
        // ainda enviando? então trava com loading global
        overlayShown = true;
        pushGlobalLoading?.();
      } catch {}
    }, SEND_MIN_MS);
    try {
      ...
      // ✅ garante que o usuário viu o "Enviando..." antes de qualquer transição
      const elapsed = Date.now() - sendStart;
      if (elapsed < SEND_MIN_MS) {
        await new Promise((r) => setTimeout(r, SEND_MIN_MS - elapsed));
      }

      // ✅ força versão e refetch
      const vNow = Date.now();
      setAvatarVersion(vNow);
      await refreshProfile?.();

      // ✅ tenta pré-carregar o bust novo (para já liberar no trade com imagem pronta)
      try {
        const base = avatarUrlRef.current || "";
        if (base) {
          const sep = base.includes("?") ? "&" : "?";
          const bust = `${base}${sep}v=${encodeURIComponent(vNow)}`;
          await preloadImage(bust, 2500);
      ...
      // ✅ requisito: liberar na tela de trade (sem modal aberto)
      try {
        requestClose();
      } catch {}
    } catch (err) {
      ...
      // ✅ cleanup timer/overlay
      try {
        if (overlayTimer) clearTimeout(overlayTimer);
      } catch {}

      // se o overlay foi aberto, fecha agora
      if (overlayShown) {
        try {
          popGlobalLoading?.();
        } catch {}

        // ✅ Ao sair do loading, dispara o efeito premium "saldo subindo" no Header
        // (UX de refresh/F5 sem mexer no saldo real)
        try {
          window.setTimeout(() => {
            try {
              window.dispatchEvent(new CustomEvent("tradepro:balance-pulse"));
            } catch {}
          }, 90);
        } catch {}
      }
              ...
              <div className={styles.dataCard}>
                <div className={styles.dataGrid}>
                  ...
                  <div className={styles.field}>
                    ...
                    <input ref={cpfInputRef} className={`${styles.input} ${fieldErrors.cpf ? styles.inputError : ""}`} value={form.cpf} onChange={(e) => setField("cpf", e.target.value)} disabled={cpfLocked} readOnly={cpfLocked} />
                    {fieldErrors.cpf ? <div className={styles.fieldError}>{fieldErrors.cpf}</div> : null}
                  </div>
                  <div className={styles.field}>
                  ...
                  <div className={styles.field}>
                    ...
                    <input ref={birthInputRef} className={`${styles.input} ${fieldErrors.nascimento ? styles.inputError : ""}`} type="date" value={form.nascimento} onChange={(e) => setField("nascimento", e.target.value)} disabled={birthLocked} readOnly={birthLocked} />
                    {fieldErrors.nascimento ? <div className={styles.fieldError}>{fieldErrors.nascimento}</div> : null}
                  </div>
                </div>
            ...
            <div className={`${styles.profileWrap} ${styles.securityWrap}`}>
              <div className={styles.securityCard}>
                <div className={styles.field}>
                  ...
                  <input ref={oldPassRef} className={`${styles.input} ${passErrors.old ? styles.inputError : ""}`} type="password" value={oldPass} onChange={(e) => { setOldPass(e.target.value); if (passErrors.old) setPassErrors((s) => ({ ...s, old: "" })); }} />
                  {passErrors.old ? <div className={styles.fieldError}>{passErrors.old}</div> : null}
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>{t("profile:security.new_password")}</label>
                  <input
                    ref={newPassRef}
                    className={`${styles.input} ${passErrors.new ? styles.inputError : ""}`}
                    type="password"
                    value={newPass}
                    onChange={(e) => {
                      setNewPass(e.target.value);
                      if (passErrors.new) {
                        // se for o erro de mismatch, limpar os dois campos para evitar duplicação/estado parcial
                        setPassErrors((s) => (isPasswordMismatchError ? { ...s, new: "", confirm: "" } : { ...s, new: "" }));
                      }
                    }}
                  />
                  {/* ✅ mostra a mensagem somente no campo "confirm" quando for mismatch */}
                  {passErrors.new && !isPasswordMismatchError ? <div className={styles.fieldError}>{passErrors.new}</div> : null}
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>{t("profile:security.confirm_password")}</label>
                  <input ref={newPass2Ref} className={`${styles.input} ${passErrors.confirm ? styles.inputError : ""}`} type="password" value={newPass2} onChange={(e) => { setNewPass2(e.target.value); if (passErrors.confirm) setPassErrors((s) => ({ ...s, confirm: "" })); }} />
                  {passErrors.confirm ? <div className={styles.fieldError}>{passErrors.confirm}</div> : null}
                </div>
                <button
                  type="button"
                  className={`${styles.btnPrimaryWide} ${!canChangePasswordFilled ? styles.disabled : ""}`}
                  onClick={doChangePassword}
                  disabled={!canChangePasswordFilled || passState === "saving"}
                >
