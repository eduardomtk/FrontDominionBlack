// src/admin/services/admin.api.js
...
/**
 * ✅ Header strategy (produção):
 * - Sempre manda apikey (gateway)
 * - Authorization SOMENTE quando houver sessão (Bearer <access_token>)
 *
 * ⚠️ Importante:
 * - NÃO use SUPABASE_ANON_KEY como Bearer fallback.
 *   Em muitos projetos ele não é JWT (ex: "sbp_...") e causa "Invalid JWT".
 */
...
// ✅ Agregado de trade_history REAL (server-side, service role)
// Agora aceita { from, to } (ISO) para respeitar range do Dashboard
...
// ✅ trades do usuário (modal) (server-side, service role)
export async function fetchAdminUserTradesREAL({ user_id, cursor = null, limit = 200 }) {
...
// ✅ agora recebe show_in_history
export async function adjustUserBalance({ user_id, account_type, delta, reason, show_in_history = false }) {
  const url = fnUrl("admin-adjust-balance");
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");

  return await tryJson(url, {
    method: "POST",
    body: JSON.stringify({ user_id, account_type, delta, reason, show_in_history: Boolean(show_in_history) }),
  });
}

// ===== Usuários (Admin) =====
export async function adminUpdateUser({ request_id, user_id, profile_patch, auth_email }) {
  const url = fnUrl("admin-user-update");
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");

  const rid = (request_id && String(request_id).trim()) || `ui_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  ...
  return await tryJson(url, {
    ...
    body: JSON.stringify({
      ...
      auth_email: auth_email ? String(auth_email).trim().toLowerCase() : null,
    }),
  });
}

export async function adminDeleteUser({ request_id, user_id, confirm_text }) {
  const url = fnUrl("admin-user-delete");
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");

  const rid =
    (request_id && String(request_id).trim()) || `ui_del_${Date.now()}_${Math.random().toString(16).slice(2)}`;
...
// ===== KYC Admin =====
export async function fetchAdminKycRequests({ status = "pending", limit = 200, offset = 0 } = {}) {
  const url = fnUrl(`admin-kyc?status=${encodeURIComponent(status)}&limit=${limit}&offset=${offset}`);
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
  return await tryJson(url);
}

export async function fetchAdminKycDetail({ request_id }) {
  const url = fnUrl(`admin-kyc-detail?request_id=${encodeURIComponent(request_id)}`);
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
  return await tryJson(url);
}
...
export async function adminBanUser({ request_id, user_id, mode, minutes, reason, note, block_email = true }) {
 ...
 *
 * Edge Functions:
 * - admin-affiliates-overview (GET)
 * - admin-affiliate-resolve   (POST)
 * - admin-affiliate-create    (POST)
 * - admin-affiliate-attach    (POST)
 * - admin-affiliate-update    (POST)
 * - admin-affiliate-delete    (POST)
 * - admin-affiliate-payouts   (GET/POST)
 * - admin-affiliate-report    (POST)
 * - admin-affiliate-referrals (GET)
 */

export async function fetchAdminAffiliatesOverview({ preset = "7d", from = null, to = null, q = null } = {}) {
...
export async function fetchAdminAffiliateResolve({ affiliate_id }) {
  ...
  return await tryJson(url, {
    method: "POST",
    body: JSON.stringify({ affiliate_id: String(affiliate_id || "").trim() }),
  });
}
...
export async function adminAffiliateUpdate({ affiliate_id, payout_pct_percent, destination_email, status }) {
...
export async function adminAffiliateDelete({ request_id, affiliate_id, delete_auth_user = true }) {
...
export async function adminAffiliateRetryPayouts({ action, affiliate_id = null, ids = [] } = {}) {
...
export async function fetchAdminAffiliateReferrals({ affiliate_id, limit = 500, offset = 0 } = {}) {
...
export async function fetchAdminAffiliateWeeklyPreview({ affiliate_id, week_end } = {}) {
  let url = fnUrl("admin-affiliate-payouts");
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
...
export async function adminAffiliateWeeklyPay({ affiliate_id, week_end, request_id = null } = {}) {
  const url = fnUrl("admin-affiliate-payouts");
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");

  const rid =
    (request_id && String(request_id).trim()) || `weekly_pay_${Date.now()}_${Math.random().toString(16).slice(2)}`;
...
export async function adminResetRankingTodayFn() {
  ...
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
  return await tryJson(url, { method: "POST", body: JSON.stringify({}) });
}
...
// ===== Support (Admin) =====
export async function fetchAdminSupportThreads({ status = "all", q = "", limit = 50 } = {}) {
  let url = fnUrl(`admin-support-threads?status=${encodeURIComponent(status)}&limit=${Number(limit || 50)}`);
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
...
export async function fetchAdminSupportThread({ thread_id }) {
  const url = fnUrl(`admin-support-thread?thread_id=${encodeURIComponent(String(thread_id || ""))}`);
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
  return await tryJson(url, { method: "GET" });
}

export async function sendAdminSupportReply({ thread_id, template_key, subject, message_text }) {
  const url = fnUrl("admin-support-send");
  if (!url) throw new Error("VITE_SUPABASE_URL não definido no .env");
  return await tryJson(url, {
    method: "POST",
    body: JSON.stringify({ thread_id, template_key, subject, message_text }),
  });
}

export async function updateAdminSupportThreadStatus({ thread_id, status }) {
