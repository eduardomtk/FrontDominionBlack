...
export default function WalletModal({
  ...
  const canDeposit = depositNumber >= 1;
  ...
  const startDeposit = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("asaas-create-pix-deposit", {
        body: {
      ...
      const providerPaymentId = data?.asaas?.paymentId || data?.deposit?.provider_payment_id || null;
      const payload = data?.asaas?.qr?.payload || null;
      const encodedImage = data?.asaas?.qr?.encodedImage || null;
      ...
      if (bonusToAttach && depositId) {
        ...
        const { error: updErr } = await supabase.from("deposits").update(upd).eq("id", depositId);
        if (updErr && import.meta.env.DEV) {
          console.warn("[WalletModal] deposits update (bonus attach) warning:", updErr?.message || updErr);
        }
      }
      setDepositStep(2);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await Promise.all([fetchLatestKyc(uid), fetchActiveBonusUsage(uid)]);
      } finally {
  ...
  const rolloverRequired = useMemo(() => {
    return getRolloverTarget(bonusUsage);
  }, [bonusUsage]);

  const rolloverCompleted = useMemo(() => {
    return getRolloverProgress(bonusUsage);
  }, [bonusUsage]);

  const rolloverProgressPct = useMemo(() => {
    if (rolloverRequired <= 0) return 0;
    return clamp(rolloverCompleted / rolloverRequired, 0, 1);
...
`.trim();
  ...
  const withdrawRequest = async () => {
    ...
    try {
      ...
      if (hasActiveBonusLock) {
        throw new Error(t("wallet:withdraw.blocked_bonus_rollover"));
      }
  ...
  const bodyScrollOn = tab === "deposit" && (depositStep === 2 || depositStep === 3);
                ...
                {depositStep === 1 && (
                  <div className={styles.depositGrid}>
                      ...
                      <>
                        <div className={styles.methodCard}>
                          <div className={styles.methodIcon} aria-hidden>
                            <svg width="92" height="28" viewBox="0 0 184 56" fill="none">
                              <path
                                d="M34.5 7c7.3 0 14.1 2.9 19.3 8.1l6.2 6.2c1.3 1.3 1.3 3.5 0 4.8l-6.2 6.2c-5.2 5.2-12 8.1-19.3 8.1s-14.1-2.9-19.3-8.1l-6.2-6.2c-1.3-1.3-1.3-3.5 0-4.8l6.2-6.2C20.4 9.9 27.2 7 34.5 7Z"
                                fill="currentColor"
                                opacity="0.92"
                              />
                              <path
                                d="M34.5 18.2c2.8 0 5.4 1.1 7.4 3.1l3.4 3.4c1.1 1.1 1.1 3 0 4.1l-3.4 3.4c-2 2-4.6 3.1-7.4 3.1s-5.4-1.1-7.4-3.1l-3.4-3.4c-1.1-1.1-1.1-3 0-4.1l3.4-3.4c2-2 4.6-3.1 7.4-3.1Z"
                    ...
                    <div className={styles.rightPane}>
                      ...
                      <button type="button" className={styles.supportLink} onClick={() => SoundManager.uiClick()}>
                    ...
                    <div className={styles.withdrawGrid}>
                      <div className={styles.withdrawCards}>
                        {bonusUsage ? (
                          <div className={styles.withdrawCard}>
                            <div className={styles.withdrawCardLabel}>
                              {t("wallet:withdraw.rollover_title")}{bonusUsage?.code ? ` (${String(bonusUsage.code).toUpperCase()})` : ""}:
                            </div>
                            ...
                            <div className={`${styles.withdrawPixHint} ${styles.withdrawPixHintExtra}`} style={{ marginTop: 10 }}>
                              {hasActiveBonusLock ? (
                                <>
                                  {t("wallet:withdraw.blocked_until_rollover")}
                                  <br />
                                  {t("wallet:withdraw.progress")}: <b>{Math.round(rolloverProgressPct * 100)}%</b>
                                </>
                              ) : (
                                <>
                                  {t("wallet:withdraw.rollover_done")}
                                  <br />
                                  {t("wallet:withdraw.unlocked")}
                                </>
                              )}
                            </div>
                            {bonusUsageError ? (
                        ...
                        <div className={styles.withdrawCard}>
                          ...
                          <div className={styles.withdrawCardValue}>R$ {formatBRL(available)}</div>
                        </div>
                        <div className={styles.withdrawCard}>
                          <div className={styles.withdrawCardLabel}>{t("wallet:withdraw.bonus")}:</div>
                          <div className={styles.withdrawCardValue}>R$ {formatBRL(bonusAmountCard)}</div>
                        </div>
                        <div className={styles.withdrawCard}>
                      ...
                      <div className={styles.withdrawPixBox}>
                        <div className={styles.withdrawPixHeader}>
                          <div className={styles.withdrawPixTitle}>PIX</div>
